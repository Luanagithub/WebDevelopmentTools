...
function init() {
    ...
    // Define o tamanho do canvas para ocupar a largura e altura total da janela
canvas.width = window.innerWidth;
canvas.height = window.innerHeight;

// Adiciona as propriedades CSS para posicionar o canvas
}
on('DOMContentLoaded', init);

	buttons.forEach(function(button) {
		button.addEventListener('mouseover', function() {
		nextPallete(); // Chame a função nextPallete() ou outra função relacionada aqui
		//adicionar comentário
	});
	});
	/*Formatação do tempo de exibição do áudio*/